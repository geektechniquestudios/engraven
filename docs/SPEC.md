...
> The complete architecture of the Engram memory system. This is the document
...
Engram's position: **memory is a retrieval problem, not a storage problem.**
...
4. **Verify or rot.** Memory that is not linted decays: links break, indexes
   drift from files, stale facts outlive their truth. Engram ships a linter
...
Engram organizes agent memory into four surfaces. Each has a distinct load
...
lines are auto-loaded** — Engram treats that as a hard budget and lints it.
...
finding things. Engram vaults use a three-tier structure for every knowledge
base (KB):
...
Not all vault knowledge is equally authoritative. Engram vaults declare a
...
Engram ships two zero-dependency linters:
...
Engram is designed to be maintained *by the agent*, per written protocol —
...
Engram is the extracted, genericized structure of a production memory system

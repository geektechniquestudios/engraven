...
Engram is the extracted structure of a production agent-memory system; the
bar for changes is "would this survive months of real agent traffic?"

- **Bugs in the linters** (`scripts/`): PRs welcome — keep them
  zero-dependency (Node ≥18 stdlib, bash + coreutils). Add a case to
  `validate-memory.sh --self-test` or the CI vault check when you fix one.

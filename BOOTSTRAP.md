# Hyphasma Bootstrap — Agent Runbook

> **You are a coding agent installing Hyphasma into a human's project.** Follow
...
1. Confirm you're inside the human's project (a git repo). If the working
   directory is the *Hyphasma clone itself*, ask the human for the path to
   their project and operate there. Call the project root `$TARGET` below;
   the Hyphasma clone `$HYPHASMA`.
...
Copy from `$HYPHASMA/template/` into `$TARGET`:

1. `template/vault/` → `$TARGET/docs/vault/` (or the path chosen in Phase 0).
   This ships with one real KB — **Hyphasma-Memory-System**, the system
...
2. `template/hyphasma.config.json` → `$TARGET/hyphasma.config.json`. Set
   `vaultDir` to the chosen path and `projectSlug` to the repo directory
   name.
3. `$HYPHASMA/scripts/vault-check.mjs` and `$HYPHASMA/scripts/validate-memory.sh`
   → `$TARGET/scripts/`. The target must be self-contained — no runtime
   dependency on the Hyphasma clone.
...
5. Optionally (**ask**): `template/.github/workflows/hyphasma-ci.yml` →
...
- If one exists: **merge** — add Hyphasma's section structure around existing
...
- **Vault router rows** ← one row for the Hyphasma KB, plus rows for the KBs
...
```
Hyphasma is installed. What you now have:
...
the changes as an `hyphasma-install` commit (let the human decide about
